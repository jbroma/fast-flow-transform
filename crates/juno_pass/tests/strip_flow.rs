...
use juno_pass::strip_flow::ReactRuntimeTarget;
use juno_pass::strip_flow::StripFlowOptions;
...
#[test]
fn pattern_annotations() {
    assert_strip(
        r#"
            const {a}: {a: number} = source;
            const [x]: Array<number> = values;
        "#,
        r#"
            const {a} = source;
            const [x] = values;
...
fn hooks() {
    ...
    assert_strip(
        r#"
            declare hook useThing(x: number): number;
            const x: number = 1;
        "#,
        r#"
            const x = 1;
...
#[test]
fn as_expressions_and_components() {
    assert_strip(
        r#"
            component View(ref?: React.RefSetter<mixed>, ...props: ViewProps) {
                const processed = props as {...ViewProps};
                const frozen = ({state: "ok"} as const);
                return processed;
            }
        "#,
        r#"
...
#[test]
fn components_ref_and_rest_lower_to_valid_js() {
    assert_strip(
        r#"
            class C {
                #x: number;
            ...
            component Foo(ref?: any, ...props: {x: number}) {
                return props.x;
            }
        "#,
        r#"
...
fn assert_strip(input: &str, expected: &str) {
    assert_strip_with_options(input, expected, Default::default());
}

fn assert_strip_with_options(input: &str, expected: &str, options: StripFlowOptions) {
    let (mut ctx_input, ast_input) = parse(input);

    let pm = PassManager::strip_flow_with_options(options);
