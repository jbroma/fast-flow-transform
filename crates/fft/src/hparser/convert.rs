...
/// Converts from Hermes AST to FFT AST
...
impl<'parser> Converter<'parser> {
    ...
    pub fn cvt_smloc(&mut self, loc: SMLoc) -> ast::SourceLoc {
        ...
        let offset = line_coord
            .line_ref
            .try_offset_from(loc.as_ptr())
            .unwrap_or_else(|| line_coord.line_ref.as_slice().len().saturating_sub(1));

        self.line_cache.make_source_loc(offset)
    }
