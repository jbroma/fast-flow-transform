...
const PLATFORM_PACKAGES = {
  'darwin-arm64': 'juno-flow-strip-loader-darwin-arm64',
  'darwin-x64': 'juno-flow-strip-loader-darwin-x64',
  'linux-arm64': 'juno-flow-strip-loader-linux-arm64',
  'linux-x64': 'juno-flow-strip-loader-linux-x64',
  'win32-arm64': 'juno-flow-strip-loader-win32-arm64',
  'win32-x64': 'juno-flow-strip-loader-win32-x64',
};
...
function stageBinaryForExecution(binaryPath) {
  if (process.env.JUNO_FLOW_STRIP_DISABLE_STAGING === '1') {
    return binaryPath;
  }
  ...
  const stagedDirectory = path.join(os.tmpdir(), 'juno-flow-strip-loader');
  ...
  const stagedPath = path.join(
    stagedDirectory,
    `juno-flow-strip-${fingerprint}${extension}`,
  );
...
function resolveFromWorkspaceBuild() {
  const executableName =
    process.platform === 'win32' ? 'juno-flow-strip.exe' : 'juno-flow-strip';
  const localPath = path.resolve(
    __dirname,
    '../../../../../unsupported/juno/target/release',
    executableName,
  );
...
function resolveBinaryPath() {
  const fromEnv = process.env.JUNO_FLOW_STRIP_BINARY;
  if (fromEnv != null && fromEnv.length > 0) {
    if (!fs.existsSync(fromEnv)) {
      throw new Error(
        `JUNO_FLOW_STRIP_BINARY points to a missing file: ${fromEnv}`,
      );
    }
  ...
  throw new Error(
    `Unable to resolve juno-flow-strip binary for ${process.platform}-${process.arch}. ` +
      'Install matching optional package or set JUNO_FLOW_STRIP_BINARY.',
  );
}
