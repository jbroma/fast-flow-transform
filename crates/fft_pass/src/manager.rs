...
impl PassManager {
    ...
    /// Pipeline containing only the Flow type stripping pass with custom options.
    pub fn strip_flow_with_options(options: strip_flow::StripFlowOptions) -> Self {
        Self {
            passes: vec![Box::new(strip_flow::StripFlow::with_options(options))],
        }
    }
