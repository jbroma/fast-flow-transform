...
use fft::gen_js;
use fft::hparser;
use fft::sourcemap::merge_sourcemaps;
...
fn test_roundtrip_with_flags(flags: hparser::ParserFlags, src1: &str) {
    ...
    for pretty in &[gen_js::Pretty::Yes, gen_js::Pretty::No] {
        ...
        dump_json(&mut dump, &mut ctx, &ast1, fft::ast::Pretty::Yes).unwrap();
        ...
        let mut dump: Vec<u8> = vec![];
        dump_json(&mut dump, &mut ctx, &ast2, fft::ast::Pretty::Yes).unwrap();
...
fn test_members() {
    ...
    test_roundtrip("class C { #x; m() { return this.#x; } }");
    test_roundtrip("class C { #x; m() { this.#x = 1; } }");
}
...
fn test_import() {
    ...
    test_roundtrip("import 'foo';");
    // The standalone Hermes parser in this repo does not currently accept import assertions.
    test_roundtrip(
        "
        import 'foo';
        import 'bar';
