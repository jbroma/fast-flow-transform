...
use fft_support::convert;
use fft_support::source_manager::SourceLoc;
...
struct GenJS<'s, 'w> {
    ...
    /// Counter used to generate temporary identifiers when lowering Flow match statements.
...
impl GenJS<'_, '_> {
    ...
    fn gen_node<'gc>(&mut self, ctx: &'gc GCLock, node: &'gc Node<'gc>, path: Option<Path<'gc>>) {
        match node {
            ...
            Node::MatchStatement(MatchStatement {
                metadata: _,
                argument,
                cases,
            }) => {
            ...
            Node::MemberExpression(MemberExpression {
                ...
                if !*computed && matches!(property, Node::PrivateName(_)) {
