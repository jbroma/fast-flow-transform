...
function main() {
  ...
  const junoRoot = repoRoot;
  const binaryName = process.platform === 'win32' ? 'juno-flow-strip.exe' : 'juno-flow-strip';
  ...
  const overrideBinaryPath = process.env.JUNO_FLOW_STRIP_BINARY;
  ...
  if (overrideBinaryPath == null || overrideBinaryPath.length === 0) {
    ...
    run('cargo', ['build', '--release', '-p', 'juno_flow_strip'], junoRoot);
  } else {
