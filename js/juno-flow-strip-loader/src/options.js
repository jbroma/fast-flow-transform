...
function parseOptions(rawOptions) {
  ...
  if (!DIALECTS.has(options.dialect)) {
    throw new Error(
      `Invalid juno-flow-strip-loader option \`dialect\`: ${String(
        options.dialect,
      )}`,
    );
  }
  ...
  if (!FORMATS.has(options.format)) {
    throw new Error(
      `Invalid juno-flow-strip-loader option \`format\`: ${String(options.format)}`,
    );
  }

  if (!REACT_TARGETS.has(String(options.reactRuntimeTarget))) {
    throw new Error(
      `Invalid juno-flow-strip-loader option \`reactRuntimeTarget\`: ${String(
  ...
  if (typeof options.enumRuntimeModule !== 'string' || options.enumRuntimeModule.length === 0) {
    throw new Error(
      'Invalid juno-flow-strip-loader option `enumRuntimeModule`: expected non-empty string',
    );
  }

  if (options.sourcemap !== true) {
    throw new Error(
      'juno-flow-strip-loader requires `sourcemap: true`; source maps are always emitted',
    );
  }
  ...
  if (options.threads != null) {
    if (!Number.isInteger(options.threads) || options.threads <= 0) {
      throw new Error(
        `Invalid juno-flow-strip-loader option \`threads\`: ${String(options.threads)}`,
      );
    }
  }
